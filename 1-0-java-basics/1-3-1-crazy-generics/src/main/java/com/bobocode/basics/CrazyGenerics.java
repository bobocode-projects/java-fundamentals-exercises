 ...
 * <p>
 * Hint: in some cases you will need to refactor the code, like replace {link T} with a generic type. In order
...
public class CrazyGenerics {
    ...
    static class CollectionUtil {
        ...
        public static <T extends BaseEntity> boolean hasDuplicates(List<T> entities, T targetEntity) {
            if (entities.isEmpty() || targetEntity == null) return false;
            int count = 0;
            for (BaseEntity e : entities) {
                if (e.getUuid().equals(targetEntity.getUuid())) count++;
                if (e.getUuid().equals(targetEntity.getUuid()) && count > 1) return true;
            }
            return false;
        }
